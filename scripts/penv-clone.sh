...
if [ $# -lt 2 ]
then
    die "Please specify the source and destination virtual environment names."
...
if [[ ! "$DEST_ENV_NAME" =~ ^[a-zA-Z0-9._-]+$ ]]; then
    die "Invalid destination environment name. Only alphanumeric characters, dots, underscores, and hyphens are allowed."
fi

# Check if source environment exists
if [ ! -d "${VENV_STORAGE_DIR}/$SOURCE_ENV_NAME" ]
then
    die "Source virtual environment '$SOURCE_ENV_NAME' does not exist."
fi

# Check if destination environment already exists
if [ -d "${VENV_STORAGE_DIR}/$DEST_ENV_NAME" ]
then
    die "Destination virtual environment '$DEST_ENV_NAME' already exists."
fi
...
if [ ! -f "$SOURCE_PYTHON" ]
then
    die "Python executable not found in source environment '$SOURCE_ENV_NAME'."
fi
...
if ! cp -r "${VENV_STORAGE_DIR}/$SOURCE_ENV_NAME" "${VENV_STORAGE_DIR}/$DEST_ENV_NAME"; then
    die "Failed to clone environment."
fi
